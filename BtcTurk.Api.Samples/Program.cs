...
class Program
{
    static async Task Main()
    {
        ...
        var wss = new BtcTurkWebSocketClient();
        wss.SetApiCredentials("9638baf8-f4d4-4f28-a729-7694e7ee0e6d", "IedU4Z+A6v0nvhWud0gf9JtyggpvE043");
        var trades = await wss.SubscribeToUserTradesAsync((data) =>
        {
            if (data != null)
            {
                Console.WriteLine($"Channel: {data.Channel} Event:{data.Event} Type:{data.Type} >> "
                    // + $"D:{data.Date} P:{data.Pair} O:{data.OrderId} S:{data.Side} A:{data.Amount} P:{data.Price} T:{data.Total} F:{data.Fee} ");
                    );
            }
        });
        ...
        var btcTurk_12 = await api.GetUserTransactionsAsync();
        ...
        // BtcTurk Websocket Api Client
        var ws = new BtcTurkWebSocketClient();
